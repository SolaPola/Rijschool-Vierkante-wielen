...
@section('title', 'Edit Car - Rijschool Vierkante Wielen')

@section('content')
    <!-- Page header -->
    <div class="flex justify-between items-center mb-6 bg-white p-4 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold text-navy-800">Edit Car</h2>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <form action="{{ route('Admin.Cars.update', $car->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Type/Model -->
                <div>
                    <label for="type" class="block text-sm font-medium text-gray-700 mb-1">Model</label>
                    <input type="text" id="type" name="type" value="{{ old('type', $car->type) }}" required
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-navy-500 focus:border-navy-500 block w-full p-2.5">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="license_plate" name="license_plate" value="{{ old('license_plate', $car->license_plate) }}" required
                           class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-navy-500 focus:border-navy-500 block w-full p-2.5 
                           @error('license_plate') border-red-500 @enderror">
                    @error('license_plate')
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center mt-4">
                        <input type="checkbox" id="isactive" name="isactive" class="h-4 w-4 text-navy-600 border-gray-300 rounded focus:ring-navy-500"
                            {{ old('isactive', $car->isactive) ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="remark" name="remark" rows="3"
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-navy-500 focus:border-navy-500 block w-full p-2.5"
                              placeholder="Any additional notes or remarks">{{ old('remark', $car->remark) }}</textarea>
                </div>
            </div>
